...
from erp.models import (
    Shipment, TrackingEvent, ReturnRequest, ShipmentStatus
)
...
class ERPService:
    ...
    def process_return(order_id: str, product_id: str, reason: str) -> Dict[str, Any]:
        ...
        return_request = ReturnRequest(
            id=return_id, order_id=order_id, items=[order_item],
            reason=reason, status="pending", refund_amount=order_item.total_price
        )
    ...
    def process_resend(order_id: str, product_id: str) -> Dict[str, Any]:
        ...
        shipment = Shipment(
            id=shipment_id, order_id=order_id, carrier="FedEx",
            tracking_number=tracking_number, status=ShipmentStatus.PROCESSING,
            estimated_delivery=estimated_delivery,
            tracking_history=[TrackingEvent(
                timestamp=datetime.now(), location="Warehouse",
                status=ShipmentStatus.PROCESSING, description="Replacement order being processed"
            )]
        )
